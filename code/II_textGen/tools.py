...
# Add path to access integration modules
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "I_integrations")))

# Import API wrappers
try:
    from web_crawling import WebCrawler  # Use the unified WebCrawler instead of individual wrappers
    from replicate_API import ReplicateAPI
    from tripo_API import TripoAPI
    from openai_API import OpenAIAPI
except ImportError as e:
    print(f"Error importing API modules: {e}")
    print("Make sure all required modules are installed and in the correct directories.")
...
class Tools:
    ...
    def __init__(self):
        ...
        try:
            self.web_crawler = WebCrawler()  # Single unified crawler instead of separate DDG and Wiki
